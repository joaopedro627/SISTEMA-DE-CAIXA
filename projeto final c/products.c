	...
	do{
		...
		switch(user_option){
			...
			case 3: printf("\nINCIANDO DESATIVAR PRODUTO ... \n");
				flag_return = desativate_element(KEY_PRODUCTS);
				if(flag_return == RETORNAR_MENU_PRINCIPAL)
					return 0;
				break;
			case 4: printf("\nINICIANDO ALTERAR PRODUTO\n\n");
				flag_return = edit_element(KEY_PRODUCTS, NULL);
				if(flag_return == RETORNAR_MENU_PRINCIPAL)
					return 0;
				break;//end case 4
			case 5:
				printf("\nINCINADO VISUALIZAR PRODUTO\n\n");
				flag_return = visualize_element(ARQ_PRODUCTS, KEY_PRODUCTS);
				if(flag_return == RETORNAR_MENU_PRINCIPAL)
					return 0;
				break;//end case 4
	...
	if(arq_product){
		...
		do{
			...
			if(loop = check_burst_buffer(p.name, MAX_PRODUCT_NAME))
				continue;
			if(verify_duplicate_name(ARQ_PRODUCTS, KEY_PRODUCTS, p.name)){
				printf("\n[X] ERRO: Produto ja existe! Cadastro cancelado.\n");
				fclose(arq_product);
			    return;
			}
			    
			if (check_similar_names(ARQ_PRODUCTS, KEY_PRODUCTS, p.name) > 0) {
			    char confirma;
